...
def check_binary_search_tree_(node):
    return check_binary_search_tree(node, 0, 100)


def check_binary_search_tree(node, mini, maxi):
    if node is None:
        return True
    if node.data < mini or node.data > maxi:
        return False
    return check_binary_search_tree(node.left, mini, node.data - 1) and check_binary_search_tree(node.right, node.data + 1, maxi)
...
if check_binary_search_tree_(tree.root):
    print('True')
else:
    print('False')
